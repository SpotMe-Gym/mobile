    ...
    <SafeAreaView className="flex-1 bg-background" edges={['top']}>
      <ScrollView className="flex-1 px-4 pt-2" contentContainerStyle={{ paddingBottom: 100 }}>
        ...
        <View className="mb-6 flex-row justify-between items-center">
          <View>
            <Text className="text-textSecondary text-sm font-medium">Monday, Jan 19</Text>
            <Text className="text-white text-3xl font-bold">Hello, {name || 'User'}</Text>
          </View>
        ...
        <View className="flex-row flex-wrap justify-between gap-y-4">
          ...
          <Card className="w-full bg-zinc-900 border border-zinc-800" title="Body Weight">
            <View className="flex-row items-baseline mt-2">
              <Text className="text-5xl font-bold text-white">{weight || '--'}</Text>
              <Text className="text-zinc-500 text-xl ml-2">kg</Text>
            </View>
            <View className="flex-row items-center mt-4">
              <TrendingUp size={16} color="#22c55e" />
              <Text className="text-green-500 ml-1 text-sm font-medium">-0.4 kg this week</Text>
            </View>
          </Card>
          ...
          >
            <Card className="h-full justify-between" title="Nutrition">
          ...
          {/* Recent Activity */}
          <Card className="w-full mt-2" title="Recent Activity">
            {/* Mock Items */}
            {[1, 2, 3].map((_, i) => (
              <View key={i} className="flex-row justify-between items-center py-3 border-b border-zinc-800 last:border-0">
                ...
                <Text className="text-zinc-400 text-sm">Completed</Text>
              </View>
            ))}
          </Card>
        </View>
